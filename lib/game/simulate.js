...
const DIFFICULTY = {
    hard: { dismissMult: 0.92, dismissFloor: 0.017, dismissCeil: 0.064, powerBoost: 1.03 },
    easy: { dismissMult: 0.68, dismissFloor: 0.012, dismissCeil: 0.047, powerBoost: 1.12 },
};
 ...
 */
export function simulateChase(battingOrder, rng = Math.random, target = TARGET, difficulty = 'hard') {
    ...
    outer: for (let over = 0; over < 20; over++) {
        ...
        for (let b = 0; b < 6; b++) {
            ...
            totalRuns += runs;
            overRuns += runs;
            ballLog.push({ over, ballInOver: b + 1, batter: striker.name, runs, isWicket: false, score: totalRuns, wickets });
